...
define('ACTION_CORRIGER_NOM', "Corriger la casse du nom");
define('ACTION_CORRIGER_PRENOM', "Corriger la casse du prénom");
...
define('ACTION_MODIFIER_PASSWORD', "Faire saisir le mot de passe !");
...
if ($AFFICHER_FORMULAIRE) {	// Récupère les valeurs du formulaire !
	...
	$nom =                      trim(getVar('nom', htmlspecialchars_decode($defaut["nom"])));
	$prenom =                   trim(getVar('prenom', htmlspecialchars_decode($defaut["prenom"]))); 
...
// Effectue les actions simples !
	$action = getVar('action', "");
        ...
        switch ($action) {
            case ACTION_CORRIGER_NOM :      $nom = ucsmart($nom);	$action=""; break;
            case ACTION_CORRIGER_PRENOM :   $prenom = ucsmart($prenom);	$action=""; break;
...
// Propose correction des noms et prénoms
	$boutonNom = ( $nom!=ucsmart($nom) ? creerFormBouton('action', ACTION_CORRIGER_NOM) : "" );
	$boutonPrenom = ( $prenom!=ucsmart($prenom) ? creerFormBouton('action', ACTION_CORRIGER_PRENOM) : "" );
	if ( $boutonNom.$boutonPrenom<>"" ) {	// Affiche l'avertissement si besoin.
		$messWarn[] = "Par convention, chaque nom &amp; prénom doit avoir sa première lettre capitale, vous pouvez utiliser le(s) bouton(s) ci-contre pour le(s) corriger.";
	}
...
// Correction du login
	...
	and ( ($login=="") or (is_null($login)) ) and ($nom!="") ) {	// Uniquement si on crée un compte info, qu'on est admin et qu'il n'est pas indiqué
		$login = nettoieAccents(strtolower($nom));
...
// Vérifie et corrige les valeurs du formulaire
	if ($action==ACTION_VALIDER) {	// Tout d'abord, on fait uniquement les contrôles et corrections
		...
		if ($lieu=="") {		$messWarn[] = "Pensez à indiquer le lieur dès que vous aurez l'information.";		}
...
// Enregistrement des données, les contrôles sont faits !
	if ($action==ACTION_VALIDER) {  var_dump($prenom); var_dump(htmlspecialchars($prenom, ENT_QUOTES, 'UTF-8', TRUE));
		$donnees = array('id_statut'=>$id_statut, 'annuaire'=>( ($annuaire=="checked") ? "1" : "0" ),
				'nom'=>$nom, 'prenom'=>$prenom, 
				//'nom'=>htmlspecialchars($nom, ENT_QUOTES, 'UTF-8', TRUE), 'prenom'=>htmlspecialchars($prenom, ENT_QUOTES, 'UTF-8', TRUE), 
				...
				if ( (is_int($result)) and ($result>0) ) {	// OK
					...
					if ($id_statut<>$statutEnregistre) {	// Gestion des envois de mails quand modifications des statuts...$demande = "création";
						...
						switch ($statutEnregistre) {	// Réglage d'une variable pour informer sur l'ancien statut du compte
                                                        ...
                                                        case 9 : $etatPrecedent = "Compte créé par le service Info, mot de passe pas encore saisi";     break;
						}
						switch ($id_statut) {	// Switch selon le nouveau statut
							...
							case 2:	// -> Annuaire	---> Passage vers annuaire : MAIL AU SERVICE INFO CAR SUPPRESSION DE COMPTE INFO
								$message = "Le compte de ".$prenom." ".$nom." (équipe ".$infoEquipe['equipe'].", du laboratoire ".$infoLabo['nom'].")";
								...
								$mailDestinataire = MAIL_SERVICE_INFO;
							...
							case 6:	// -> Demande de suppression	---> Suspension ou suppression, demandée : MAIL AU SERVICE INFO CAR SUPPRESSION A FAIRE
								$message = "Une demande de <strong>".$demande."</strong> de compte vient d'être faite par ".getenv('SSL_CLIENT_S_DN_CN')." !\r\n";
								$message.= "Il s'agit de ".$prenom." ".$nom.",\r\n";
								$message.= "qui fait partie de l'équipe ".$infoEquipe['equipe'].", du laboratoire ".$infoLabo['nom'].",\r\n";
								$message.= "La ".$demande." doit prendre effet à partir de ".dateConv($date_depart, DATE_FORMAT_HUMAIN).".\r\n\r\n\r\n";
								$message.= "Validez la demande par ce lien : <a href='".$lienCompte."'>".$lienCompte."</a>\r\n\r\n";
								$mailExpediteur = "<".$AUTORISATION_MAIL.">";
								$mailDestinataire = MAIL_SERVICE_INFO;
								$mailEntete = 'MIME-Version: 1.0'."\r\n";
								$mailEntete.= 'Content-type: text/html; charset=utf-8'."\r\n";
								...
								$mailEntete.= "Cc: ".$mailExpediteur."\r\n";
								$mailEntete.= "Reply-to: ".$mailExpediteur."\r\n";
								$mailSubject =	"[GestWEB] Demande de ".$demande." de compte";
								if ( mail($mailDestinataire, $mailSubject, str_replace("\r\n", "<br />", $message), $mailEntete) ) {
									$messWarn[] = "Un message a été envoyé au service info pour qu'ils suspendent ou suppriment le compte.";
								} else {
									$messErr[] = "Le message n'a pas pu être envoyé au service info : contactez-les !";
								}
							break;
							...
							case 3:	//  Compte valide
								switch ($statutEnregistre) {
										...
										if ($inscriptionListeSeminaire=="checked") {	// Inscription à la liste des séminaires
											$mailExpediteur = $email;
											$mailEntete = 'MIME-Version: 1.0'."\r\n";
											$mailEntete.= 'Content-type: text/html; charset=utf-8'."\r\n";
											$mailEntete.= "From: ".$email."\r\n";
											$mailEntete.= "Reply-to: ".$email."\r\n";
											$mailSubject =	"SUBSCRIBE cbi.int-seminars ".$prenom." ".$nom;
											if ( mail("sympa@univ-tlse3.fr", $mailSubject, "", $mailEntete) ) {
		...
		.\r\n";
										...
										$message.= "Votre compte sera actif du ".dateConv($date_arrivee, DATE_FORMAT_HUMAIN)." ";
										$message.= ( $date_depart==""?",sans limite de durée":"au ".dateConv($date_depart, DATE_FORMAT_HUMAIN)).".\r\n";
										$message.= "Il vous appartient de nous signaler ".( $date_depart==""?"votre date de départ":"toute prolongation de contrat").".\r\n\r\n";
										if ($inscriptionListeSeminaire=="checked") {	// Vérification si l'inscription est faite
											...
											$message.= " <a href='mailto:sympa@univ-tlse3.fr?subject=UNSUBSCRIBE%20cbi.int-seminars%20".$prenom."%20".$nom."'>";
											$message.= " mailto:sympa@univ-tlse3.fr?subject=UNSUBSCRIBE%20cbi.int-seminars%20".$prenom."%20".$nom."</a>\r\n";
											...
											$message.= " <a href='mailto:sympa@univ-tlse3.fr?subject=SUBSCRIBE%20cbi.int-seminars%20".$prenom."%20".$nom."'>";
											$message.= " mailto:sympa@univ-tlse3.fr?subject=SUBSCRIBE%20cbi.int-seminars%20".$prenom."%20".$nom."</a>\r\n";
										}
										$mailExpediteur = MAIL_SERVEUR_INFO;
										$mailDestinataire = $prenom." ".$nom." <".$email.">";
										$mailEntete = 'MIME-Version: 1.0'."\r\n";
										$mailEntete.= 'Content-type: text/html; charset=utf-8'."\r\n";
										$mailEntete.= "From: ".$mailExpediteur."\r\n";
										$mailEntete.= "Cc: ".$mailExpediteur."\r\n";
										$mailEntete.= "Reply-to: ".$mailExpediteur."\r\n";
										$mailSubject =	"[GestWEB] Bienvenue a l'IBCG";
										if ( mail($mailDestinataire, $mailSubject, str_replace("\r\n", "<br />", $message), $mailEntete) ) {
											...
											$messErr[] = "Le message n'a pas pu être envoyé au titulaire : vérifiez !";
                                                                                ...
                                                                                $message.= "Il s'agit de ".$prenom." ".$nom.",\r\n";
                                                                                $message.= "qui fait partie ";
                                                                                if ($id_equipe>0) {  $message.= "de l'équipe ".$infoEquipe['equipe'].", ";  }
                                                                                $message.= "du laboratoire ".$infoLabo['nom'].",\r\n";
                                                                                $message.= "qui sera joignable au poste ".$poste." et être ".$infoEmploi['job']." (".$infoEmploiCategorie['categorie'];
                                                                                $message.= "), dépendant de ".$infoAppartenance['appartenance'].".\r\n";
                                                                                $message.= "Son compte sera actif à partir du ".dateConv($date_arrivee, DATE_FORMAT_HUMAIN);
                                                                                $message.= ( $date_depart==""?",sans limite de durée":" et jusqu'au ".dateConv($date_depart, DATE_FORMAT_HUMAIN)).".\r\n\r\n\r\n";
                                                                                $messageTechnik = $message."Vous pouvez lui fournir un badge pour la durée indiquée.\r\n\r\n";
                                                                                $mailExpediteur = "<".$AUTORISATION_MAIL.">";
                                                                                $mailEntete = 'MIME-Version: 1.0'."\r\n";
                                                                                $mailEntete.= 'Content-type: text/html; charset=utf-8'."\r\n";
                                                                                $mailEntete.= "From: ".$mailExpediteur."\r\n";
                                                                                ...
                                                                                if (in_array($id_labo, array(1,2,8))) { // Envoi uniquement si labo LBME, LMGM ou CBI !
                                                                                    if ( mail("Assistants de Prevention <ibcg.ap@ibcg.biotoul.fr>", $mailSubject, str_replace("\r\n", "<br />", $message), $mailEntete) ) {
                                                                                ...
                                                                                if (in_array($id_labo, array(1,2,8))) { // Envoi uniquement si labo LBME, LMGM, CBI, CBD, LBCMCP ou CRCA !
                                                                                    if ( mail("Service Technique <technik@ibcg.biotoul.fr>", $mailSubject, str_replace("\r\n", "<br />", $messageTechnik), $mailEntete) ) {
									...
									case 5: case 7: case 8:	// Revalidation d'un compte suspendu ou supprimé : mail au titulaire
										...
										$message.= ( $date_depart==""?"sans limite de durée":"jusqu'au ".dateConv($date_depart, DATE_FORMAT_HUMAIN)).".\r\n";
										$message.= "Il vous appartient de nous signaler ".( $date_depart==""?"votre date de départ":"toute prolongation de contrat").".\r\n\r\n";
										$mailExpediteur = MAIL_SERVEUR_INFO;
										$mailDestinataire = $prenom." ".$nom." <".$email.">";
										$mailEntete = 'MIME-Version: 1.0'."\r\n";
									...
									case 4: case 6: // Annulation d'une demande de suspension ou suppression : mail au service info
										$message = "Une demande de ".$demande." de compte vient d'être <strong>annulée</strong> par ".getenv('SSL_CLIENT_S_DN_CN')." !\r\n";
										$message.= "Il s'agit de ".$prenom." ".$nom.",\r\n";
										$message.= "qui fait partie de l'équipe ".$infoEquipe['equipe'].", du laboratoire ".$infoLabo['nom'].",\r\n";
										$message.= "Consultez le compte par ce lien : <a href='".$lienCompte."'>".$lienCompte."</a>\r\n\r\n";
										$mailExpediteur = "<".$AUTORISATION_MAIL.">";
										$mailDestinataire = MAIL_SERVICE_INFO;
										$mailEntete = 'MIME-Version: 1.0'."\r\n";
										$mailEntete.= 'Content-type: text/html; charset=utf-8'."\r\n";
			...
			if ( (is_int($result)) and ($result>0) ) {	// OK
				...
				if ($donnees['id_statut']==1) {	// On a ajouté un compte : Envoi du mail aux administrateurs...
					...
					$infoLabo = sqlTrouve('reseau.labos', 'idx='.$id_labo);
					$infoEquipe = sqlTrouve('reseau.equipes', 'idx='.$id_equipe);
					$infoEmploi = sqlTrouve('emploi', 'id_job='.$id_job);
					$infoEmploiCategorie = sqlTrouve('emploi_categorie', 'idx='.$infoEmploi['id_categorie']);	//var_dump($id_appartenance);
					$infoAppartenance = sqlTrouve('appartenance', 'idx='.$id_appartenance);						//var_dump($infoAppartenance);
					$message = "Une demande de compte vient d'être validée par ".getenv('SSL_CLIENT_S_DN_CN')." !\r\n";
					...
					$message.= "qui sera joignable au poste ".$poste." et être ".$infoEmploi['job']." (".$infoEmploiCategorie['categorie'];
					$message.= "), dépendant de ".$infoAppartenance['appartenance'].".\r\n";
					$message.= "Son compte devra rester actif ";
					$message.= ( $date_depart==""?"sans limite de durée":"jusqu'au ".dateConv($date_depart, DATE_FORMAT_HUMAIN)).".\r\n\r\n\r\n";
					$message.= "Merci de créer le compte correspondant, il ";
					$message.= ($annuaire=="checked"?"sera ajouté à l'annuaire IBCG après validation.":"ne sera pas ajouté à l'annuaire IBCG").".\r\n\r\n";
					$message.= "Validez la demande par ce lien : <a href='".$lienCompte."'>".$lienCompte."</a>\r\n\r\n";
					$mailExpediteur = "<".$AUTORISATION_MAIL.">";
...
if ($AFFICHER_FORMULAIRE) {	// Affichage du formulaire
	...
	echo("<form method='post' action='' enctype='multipart/form-data'>");
			...
			if ($idvar>0) {	// Affiche la liste déroulante UNIQUEMENT pour les comptes existants (pas en création)
				...
				if ($AUTORISATION==2) {	// Pour administrateur :
					switch ($id_statut) {
						...
						break;
						case 5:	case 7:	case 8:	// Compte suspendu, supprimé, ou revalidation demandée
		...
		// Nom
			echo("<tr><th> Nom </th>");
			echo("<td> ".champTexte('nom', 'Indiquez le nom (obligatoire !)', $nom, 20, $MODIFIABLE)."&nbsp;".$boutonNom." </td>");
		// Prénom
			echo("<tr><th> Prénom </th>");
			echo("<td> ".champTexte('prenom', 'Indiquez le prénom (obligatoire !)', $prenom, 20, $MODIFIABLE)."&nbsp;".$boutonPrenom." </td></tr>");
		// Login
			...
			echo( champTexte('login', 'Indiquez le login'.(($id_statut<>2)?' (obligatoire !)':''), $login, 15, (($MODIFIABLE)&&($AUTORISATION==2))) );	// Spécifique : modifiable que pour admin !
                        if ( ($AUTORISATION==2) and ($id_statut!=2) and ($login<>"") ) {
                            echo(formBouton('action', ACTION_MODIFIER_PASSWORD));
                        }
			echo(" </td>");
		// Labo
                        ...
                        if ($MODIFICATION_EQUIPE) { // On permet de modifier la responsabilité et la quotité, mais on affichera que les boutons Détacher et Valider les modifs
                                ...
                                foreach ($equipesLabos as $une) {
                                        if ($ex_labo!=$une['nom_labo']) {
                                            $listeEquipes['DISABLED'.$une['nom_labo']] = '---'.$une['nom_labo'];
		...
		// Abonnements aux liste de diffusion
			if ($id_statut==1) {	// Proposé uniquement si le compte est à créer
				echo("<tr><th> Abonnements aux listes de diffusion </th><td>");
		...
		// Boutons
			...
			echo("<p>Après avoir pris connaissance de la charte informatique, remettez ce formulaire au service informatique.<br />");
			echo("Vous devrez alors saisir le mot de passe de votre compte. Il doit respecter les règles suivantes :</p>");
			echo("<ul><li>Avoir entre 10 et 16 caractères,</li>");
			echo("<li>Comporter au moins 3 alphabets sur les 4 (minuscules, majuscules, chiffres, caractères spéciaux),</li>");
			echo("<li>Ne pas comporter de lettres accentuées,</li>");
			echo("<li>Ne pas comporter vos nom ou prénom, ni de mot pouvant être présent dans un dictionnaire.</li></ul>");
			...
			echo("<p>Date et signature, précédées de la mention manscrite &quot;lu et approuvé&quot; :<br /><br /><br /></p>");
			echo("</span>");
