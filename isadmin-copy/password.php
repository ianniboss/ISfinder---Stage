		...
		switch ($action) {
			...
			case BOUTON_OK_CHANGE:
				...
				if (count($messageErreur)==0) {		// Toutes les vÃ©rifications sont passÃ©es avec succÃ¨s : essai de changer le mot de passe
					...
					$passwd = str_replace($carDebut,$carFin,$champMotPasseNouveau); $messageErreur[] = "Mot de passe : ".$champMotPasse;
					$sshPass = str_replace($carDebut,$carFin,SSH_pass);              $messageErreur[] = "Mot de passe compte : ".$passwd;
	...
	?>
		...
		<ul>
			...
			<li> contenir au moins 3 classes de caract&egrave;res (minuscules, majuscules, chiffres, caract&egrave;res s&eacute;ciaux), </li>
                        <li> ne pas contenir plus de 3 caract&egra&egrave;res consécutifs de la même classe, </li>
