        ...
        if (!mysqli_select_db($lien, DB_bdd)) {
            ...
            if (!$resultat) {
                ...
                if (stristr($requeteSQL, "INSERT") !== FALSE) {
                    $retour = mysqli_insert_id($lien);
                }
                if ((stristr($requeteSQL, "UPDATE") !== FALSE) || (stristr($requeteSQL, "DELETE") !== FALSE)) {
                    $retour = mysqli_affected_rows($lien);
                }
            }
        }
        mysqli_close($lien);
...
function sqlSelect($table, $condition = '', $tri = '', $limite_ou_groupe = null) {
    ...
    if ($tri != '') {
        $sql .= ((stristr($tri, "GROUP BY") === FALSE) ? " ORDER BY " : " ") . $tri;
    }
    if ((!is_null($limite_ou_groupe)) and (is_numeric($limite_ou_groupe))) {
        $sql .= ' LIMIT ' . $limite_ou_groupe;
    }
    $sql .= ' ;';    // echo($sql.'<br />');
    // récupération des résultats SI possible...
...
function sqlTrouve($table, $condition = '', $tri = '', $groupe = null) {
    ...
    if (!$resultat) {
        return('ERREUR ' . mysqli_error($cnx) . ' : Impossible d\'exécuter la requète (' . $sql . ').');
    }
    ...
    if (is_array($donnees)) {
        ...
        foreach ($donnees as $cle => $valeur) {
            ...
            if (is_null($valeur)) {
                ...
                $liste_valeurs .= "MD5('" . $valeur . "')";
            } else if (!is_numeric($valeur)) {
                $liste_valeurs .= '"' . addslashes($valeur) . '"';
            } else {
