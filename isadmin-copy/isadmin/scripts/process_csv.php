...
if (empty($raw_sql)) {
    die("La requ&ecirc;te SQL est vide.");
}

// SECURITY VALIDATION 1: Must start with SELECT
if (stripos($raw_sql, 'SELECT') !== 0) {
    die("S&eacute;curit&eacute; : Seules les requ&ecirc;tes SELECT sont autoris&eacute;es.");
}

// SECURITY VALIDATION 2: No semicolons to prevent multiple statements
if (strpos($raw_sql, ';') !== false) {
    die("S&eacute;curit&eacute; : L'utilisation de points-virgules (;) est interdite pour emp&ecirc;cher les requ&ecirc;tes multiples.");
}
...
foreach ($blacklist as $keyword) {
    ...
    if (preg_match('/\b' . $keyword . '\b/', $upper_sql)) {
         die("S&eacute;curit&eacute; : L'utilisation du mot-cl&eacute; " . htmlspecialchars($keyword) . " est strictement interdite.");
    }
}
...
if ($result === false) {
    ...
    mysqli_close($lien);
    die("Erreur SQL : " . htmlspecialchars($error));
}

// It must be a result set from a SELECT
if (!($result instanceof mysqli_result)) {
    mysqli_close($lien);
    die("S&eacute;curit&eacute; : La requ&ecirc;te n'a pas retourn&eacute; de jeu de r&eacute;sultats.");
}
