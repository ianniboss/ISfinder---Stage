...
# On ne veut pas de cochoncetés dans la paramètre id!!!
	$id_fichier=strip_tags($_GET['id']);
	$fichier = "/var/www/uploads/blast/tmp/".$id_fichier.".res";

	if ((strlen($id_fichier) == 9) and (!ereg('[^A-Za-z0-9]',$id_fichier)) and (file_exists($fichier))) {

		$command="ps -ef|grep ".$id_fichier."[.]";
		$resultat=exec($command);
		if (!$resultat) {
			echo "</head><body  bgcolor=\"#F8E9C2\"><h3>BLAST search result</h3>";
#			echo "<font size=\"2\"><a href=\"resultatam.php?id=".$id_fichier."\">Enhanced view</a></font><br>";
		...
		echo "<p>If you think it's server error, please contact the <a href=\"mailto:webadmin@ibcg.biotoul.fr\">webmaster</a>.</p></body></html>";
	}
?>
