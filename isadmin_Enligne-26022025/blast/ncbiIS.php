...
# Cas ou la sequence est copiée/collée
...
# Cas ou la sequence est uploadée
...
# Sinon, il faut absolument une séquence!
	} else {
...
# aggrégation des arguments du blastall
...
# La commande finale, tout est redirigé dans un fichier pour avoir aussi les messages d'erreurs.

			$finalcom="/var/www/html/secure/isadmin/blast/bin/".$command." >& $fichfinal &";
			$result = exec($finalcom);
			}
			$nomfich=substr($fichresult,27);
# Le process est lancé, on affiche une page temporaire, qui au bout de 2 secondes charge resutat.php
			echo "<html><head><meta http-equiv='refresh' content='2;";
			if ($_POST['alignment']==0){
				echo "URL=https://secure.ibcg.biotoul.fr/isadmin/blast/resultat.php?id=$nomfich&database=$database&title=$title'></head>\n";
				echo "<body><h2>Please wait, your request is in progress...$nomfich</h2>\n";
				echo "<p>If automatic refresh doesn't work, please click ";
				echo "<a href=\"https://secure.ibcg.biotoul.fr/isadmin/blast/resultat.php?id=$nomfich&database=$database&title=$title\">here...</a></p></body></html>\n";
			}else{
				echo "URL=https://secure.ibcg.biotoul.fr/isadmin/blast/resultatnorm.php?id=$nomfich&title=$title'></head>\n";
			}

//			echo "<a href=\"https://secure.ibcg.biotoul.fr/isadmin/blast/resultatnorm.php?id=$nomfich&title=$title\">here...</a></p></body></html>\n";
		}
