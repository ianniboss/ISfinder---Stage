<?php
// V�rification du Referer pour les variables pass�es en POST
...
{
	if ($base == "ISfinder"){
		$connecte=mysqli_connect("localhost","isfinder","JmCjMPEPat_16!foB","ISfinder") or die("Server connection error");
	}elseif ($base == "ISsubmit"){
		$connecte=mysqli_connect("localhost","issubmit","mCj#MPEJ_16!*","ISsubmit") or die("Server connection error");
	/*	$connecte=mysqli_connect("astun.ibcg.biotoul.fr","issubmit","mCjMPEJ_16","ISsubmit") or die("Server connection error"); */
	}else{
		die("Server connection error");
	}

mysqli_select_db($connecte,$base) or die("connection error to the database".$base);
...
/*		Indication du type d'erreur pour d�bogger */
/*		$erreur_no = mysqli_errno($connect);
		$erreur_txt = mysqli_error($connect);
		if ($erreur_no == "1062") {
        ...
        echo "ERROR :".mysqli_errno($connect).":".mysqli_error($connect)."<br>\n";
		}
*/
    exit;
    }
...
//________Requ�te pour r�cup�rer la valeur num�rique de MGE Type _____________
...
//______formatage de l'affichage d'une s�quence__________________________
