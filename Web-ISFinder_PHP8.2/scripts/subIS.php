...
if (empty($_POST)) {
    $_SESSION["error"] = "Problem with the data. \n Please contact the webadmin";
}
...
if ($form_soumis === "Submit") {
    ...
    // Captcha verification (before any other processing)
    require_once("ptitcaptcha.php");
    if (!PtitCaptchaHelper::checkCaptcha()) {
        $_SESSION["captcha_error"] = "The anti-spam code entered was incorrect. Please try again.";
        // Save all form data to session so the user doesn't lose their input
        foreach ($_POST as $key => $value) {
            $_SESSION[$key] = trim(stripslashes(htmlentities($value)));
        }
        header("Location: /submission.php?raz=0");
        exit();
    }

    /* Initialisation of the error variable */
    $_SESSION["error"] = "";

    /* Fields sanitization and retrieval */
    foreach ($_POST as $champ => $valeur) {
        $$champ = $_SESSION[$champ] = trim(stripslashes(htmlentities($valeur)));
    }
    ...
    /* DNA and Protein sequences character cleaning */		
    $car_elim = ["\n", "\r", " "];
    ...
    if ($_SESSION["error"] === "") {
        ...
        // Ends calculations and Table insertion
        ...
        // Confirmation emailsCurator of ISFinder
        $signature = "\r\n\r\nRegards\r\nPatricia Siguier\r\n\r\n------------------\r\nPatricia Siguier\r\n";
