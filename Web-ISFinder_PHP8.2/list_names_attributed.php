...
$host = (!empty($_GET['host']) && ($_GET['host']) && in_array($_GET['host'],$tab_hostGere)) ? intval($_GET['host']) : '1';
...
<!--    Choix du type d'hote : on recharge la page avec lettre et bestiole déjà sélectionnée si changement d'hote (Bacteria / Meta data / Virus)   -->        
        ...
        <ul class="bouton_ligne">
            ...
            <li>
               ...
               <label for=mite>Metagenomic data</label>
            </li>
            <li>
...
<li>        <input type="radio" name="champ" value="names" />Names<br />  </li>
    		</form>      
		</fieldset>
        </div>        
 <fieldset>
<?php
// Affichage des lesttres A-Z avec lien pour chaque lettre, rechargeant la page avec lettre, type de bestiole sélectionné, et type d'hôte
...
// Affichage résultat
	If ($nombre > 0) {
		...
		for ($i=0;$i<$nombre;$i++){			
			...
			if ($bact_origin_prec != $bact_origin && $i >0) { print "<tr><td colspan = 7><HR></td></tr>" ; }			
